...
public class TopTenAirportsReducer extends Reducer<Text, Text, Text, Text> {
	...
	private TreeMap<Long, String> airportWeatherDelays;

	@Override
	protected void setup(Context context) {
		busiestAirportsBefore1998 = new TreeMap<>();
		busiestAirportsOnOrAfter1998 = new TreeMap<>();
		airportWeatherDelays = new TreeMap<>();
	}

    @Override
    protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		// Will 3 elements if it is related to question 3, else 2 elements if related to question 6
		...
		if (question.equals("q3")) {
			...
			for (Text val : values) {
				...
				airportWeatherDelays.put(count, airport);
			}
		}
    }
	...
	protected void cleanup(Context context) throws IOException, InterruptedException {
		...
		TreeMap<Long, String> reversedAirportWeatherDelays = new TreeMap(Collections.reverseOrder());
		reversedAirportWeatherDelays.putAll(airportWeatherDelays);

		context.write(new Text("\nTop 10 cities with the most weather-related delays"), new Text());

		int counter3 = 0;
		for (Map.Entry<Long, String> count_airport : reversedAirportWeatherDelays.entrySet()) {
			if (counter3 == 10)
				break;
